...
def _read_or_empty(path: str = "synth.metadata", alogger=None):
    """Reads a metadata json file.  Returns empty if that file is not found."""
    try:
        if not alogger:
            alogger = logger  # Use the global logger if none passed in.
        with open(path, "rt") as file:
            text = file.read()
        metadata = google.protobuf.json_format.Parse(text, metadata_pb2.Metadata())
        if metadata.metadata_version > METADATA_VERSION:
            alogger.critical(
                "%s was generated by a newer version of synthtool.  "
                + "Please update synthtool to the latest version resolve.", path)
        return metadata
...
def write(outfile: str = "synth.metadata") -> None:
    ...
    if _metadata.metadata_version <= 0:
        _metadata.metadata_version = METADATA_VERSION
...
class MetadataTrackerAndWriter:
    ...
    # _thread_local.live_tracker points to the MetadataTrackerAndWriter in the current
    #  thread context.
